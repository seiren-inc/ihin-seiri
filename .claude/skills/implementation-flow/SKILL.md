...
Use for: new features, new components, new routes, new API endpoints, any substantive code addition. Do not start coding without completing the plan and receiving approval.

## Workflow

**Phase 1 — Analysis**
1. Read all relevant existing files (page, component, types, lib)
...
### Files to modify
- src/components/sections/X.tsx — [reason]

### Files to create
- src/components/sections/Y.tsx — [reason]
...
- Use `@/*` path aliases
- Follow `src/` directory conventions (app/, components/ui|common|sections|layout/, lib/)
- All CMS fetches in Server Components using `src/lib/microcms.ts`
